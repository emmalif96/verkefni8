...
const text = (() => {
  ...
  function init(_form, _items) {
    ...
    for (let item of _items.querySelectorAll('.item__checkbox')) {
      item.addEventListener('click', finish);
    }
    for (let item of _items.querySelectorAll('.item__button')) {
        item.addEventListener('click', deleteItem);
      }
    for (let item of _items.querySelectorAll('.item__text')) {
  ...
  function edit(e) {
    let inntak = el("INPUT", CLASSES.TEXT);
    inntak.value = e.target.textContent; 
    inntak.addEventListener("keyup" , (e) => {
  ...
  function add(value) {
    ...
    const button = el("BUTTON", CLASSES.BUTTON, deleteItem);
